...
public class SellerOrderAsync {
    private Activity activity;

    private RequestModel requestModel;

    public SellerOrderAsync(final Activity activity, RequestModel requestModel) {
        ...
        if (Utility.checkInternetConnection(activity)) {
            ...
            try {
                ...
                Call<ResponseModel> call = apiService.getsellerOrderListResponse(jsonObject.toString());
    ...
    private void onpostExecute(ResponseModel response) {
        if (response != null)
        {
            ...
            ((Seller_OrderActivity) activity).successresponse(response);

        }
        else
        {
            ((Seller_OrderActivity) activity).progressDialogdismiss();
